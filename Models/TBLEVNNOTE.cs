using System;
using System.Collections.Generic;

namespace FLEXCrm.DEMODATA
{
    public partial class TBLEVNNOTE
    {
        public string EID { get; set; } = null!;
        public string? EINV { get; set; }
        public string RNSTATUS { get; set; } = null!;
        public string? EVENTNOTES { get; set; }
        public string? BILLAPPLICABLE { get; set; }
        public double? BILLPERCENT { get; set; }
        public DateTime DATED { get; set; }
        public bool? PI { get; set; }
        public bool? CL { get; set; }
        public bool? PA { get; set; }
        public bool? RL { get; set; }
        public bool? AM { get; set; }
        public string? OPTION1 { get; set; }
        public string? OPTION2 { get; set; }
        public string? OPTION3 { get; set; }
        public string? OPTION4 { get; set; }
        public string? OPTION5 { get; set; }
        public string? UPSTATUS { get; set; }
        public string? DNSTATUS { get; set; }
        public string? USRID { get; set; }
        public string? BID { get; set; }
    }
}
